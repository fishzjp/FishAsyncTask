...
class PerformanceMetrics:
    ...
    def get_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "latency_stats_ms": {k: v * 1000 for k, v in latency_stats.items()},
...
def measure_concurrent_performance(
    func: Callable, args_list: List[Any], max_workers: int = 100, description: str = "并发操作"
) -> PerformanceMetrics:
    """
    测量并发操作的性能
...
def compare_results(baseline: Dict[str, Any], current: Dict[str, Any]) -> Dict[str, Any]:
    ...
    latency_improvement = (
        ((baseline_p99 - current_p99) / baseline_p99 * 100) if baseline_p99 > 0 else 0
    )
...
def print_comparison_report(
    scenario: str, baseline: Dict[str, Any], current: Dict[str, Any], comparison: Dict[str, Any]
) -> None:
    """
    打印性能对比报告
    ...
    print("-" * 65)
    print(
        f"{'QPS':<20} {comparison['baseline_qps']:<15.0f} {comparison['current_qps']:<15.0f} {comparison['qps_improvement_percent']:+.1f}%"
    )
    print(
        f"{'P99 延迟(ms)':<20} {comparison['baseline_p99_ms']:<15.3f} {comparison['current_p99_ms']:<15.3f} {comparison['latency_improvement_percent']:+.1f}%"
    )
    print(
        f"{'成功率':<20} {baseline['success_rate']*100:<14.2f}% {current['success_rate']*100:<14.2f}% {'0.0%':<15}"
    )
    print("-" * 65)
