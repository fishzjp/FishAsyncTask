...
class TestComprehensiveComparisonReport:
    ...
    def _print_comparison_table(
        self, title: str, headers: List[str], rows: List[List[Any]]
    ) -> None:
        """
        打印对比表格
    ...
    def test_comprehensive_throughput_comparison(self, fish_task_manager, redis_available):
        ...
        rows.append(
            [
                "吞吐量 (tasks/s)",
                f"{results['FishAsyncTask']['throughput']:.0f}",
                f"{results['concurrent.futures']['throughput']:.0f}",
            ]
        )
        if "huey" in results:
        ...
        rows.append(
            [
                "QPS",
                f"{results['FishAsyncTask']['qps']:.0f}",
                f"{results['concurrent.futures']['qps']:.0f}",
            ]
        )
        if "huey" in results:
        ...
        rows.append(
            [
                "P99 延迟 (ms)",
                f"{results['FishAsyncTask']['p99_latency_ms']:.3f}",
                f"{results['concurrent.futures']['p99_latency_ms']:.3f}",
            ]
        )
        if "huey" in results:
        ...
        rows.append(
            [
                "平均延迟 (ms)",
                f"{results['FishAsyncTask']['avg_latency_ms']:.3f}",
                f"{results['concurrent.futures']['avg_latency_ms']:.3f}",
            ]
        )
        if "huey" in results:
        ...
        rows.append(
            [
                "成功率 (%)",
                f"{results['FishAsyncTask']['success_rate']:.1f}",
                f"{results['concurrent.futures']['success_rate']:.1f}",
            ]
        )
        if "huey" in results:
        ...
        rows.append(
            [
                "内存增量 (MB)",
                f"{results['FishAsyncTask']['memory_mb']:.1f}",
                f"{results['concurrent.futures']['memory_mb']:.1f}",
            ]
        )
        if "huey" in results:
