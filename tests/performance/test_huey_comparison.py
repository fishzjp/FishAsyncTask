...
class TestHueyComparison:
    ...
    def test_task_submission_throughput(self, huey_manager, fish_task_manager):
        ...
        print(
            f"{'P99 延迟 (ms)':<25} {fish_metrics.get_percentile(99) * 1000:<20.3f} {huey_metrics.get_percentile(99) * 1000:<20.3f}"
        )
        print(
            f"{'平均延迟 (ms)':<25} {fish_metrics.get_avg_latency() * 1000:<20.3f} {huey_metrics.get_avg_latency() * 1000:<20.3f}"
        )
        print(
            f"{'内存增量 (MB)':<25} {mem_after_fat - mem_before_fat:<20.1f} {mem_after_huey - mem_before_huey:<20.1f}"
        )
        print(
            f"{'成功率 (%)':<25} {fish_metrics.get_success_rate() * 100:<19.1f}% {huey_metrics.get_success_rate() * 100:<19.1f}%"
        )

        # 计算性能差异
        throughput_diff = (
            ((fat_throughput - huey_throughput) / huey_throughput * 100)
    ...
    def test_concurrent_status_query(self, huey_manager, fish_task_manager):
        ...
        print(
            f"{'P50 延迟 (ms)':<25} {fish_metrics.get_percentile(50) * 1000:<20.3f} {huey_metrics.get_percentile(50) * 1000:<20.3f}"
        )
        print(
            f"{'平均延迟 (ms)':<25} {fish_metrics.get_avg_latency() * 1000:<20.3f} {huey_metrics.get_avg_latency() * 1000:<20.3f}"
        )
        ...
        # 计算性能差异
        qps_diff = (
            ((fish_metrics.get_qps() - huey_metrics.get_qps()) / huey_metrics.get_qps() * 100)
            if huey_metrics.get_qps() > 0
            else 0
        )
        print(f"\n性能差异分析:")
    ...
    def test_cleanup_performance(self, huey_manager, fish_task_manager):
        ...
        """
        print_test_header(
            "huey vs FishAsyncTask - 清理操作性能", task_count=TestConfig.TASK_COUNT_LARGE
        )
        ...
        print(
            f"{'清理耗时 (ms)':<25} {fish_cleanup_time * 1000:<20.3f} {huey_cleanup_time * 1000:<20.3f}"
        )
        if huey_throughput > 0:
    ...
    def test_end_to_end_performance(self, huey_manager, fish_task_manager):
        ...
        """
        print_test_header(
            "huey vs FishAsyncTask - 端到端性能", task_count=TestConfig.TASK_COUNT_MEDIUM
        )
        ...
        print("-" * 65)
        print(
            f"{'任务执行时间 (秒)':<25} {fish_execution_time:<20.3f} {huey_execution_time:<20.3f}"
        )
        print(f"{'总耗时 (秒)':<25} {fish_total_time:<20.3f} {huey_total_time:<20.3f}")
        print(
            f"{'任务吞吐量 (tasks/s)':<25} {len(fish_task_ids) / fish_execution_time:<20.0f} {len(huey_async_results) / huey_execution_time:<20.0f}"
        )
        print(
            f"{'状态查询 QPS':<25} {fish_query_metrics.get_qps():<20.0f} {huey_query_metrics.get_qps():<20.0f}"
        )
        print(
            f"{'状态查询 P99 (ms)':<25} {fish_query_metrics.get_percentile(99) * 1000:<20.3f} {huey_query_metrics.get_percentile(99) * 1000:<20.3f}"
        )
