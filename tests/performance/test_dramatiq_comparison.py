...
class TestDramatiqComparison:
    ...
    def test_task_submission_throughput(self, dramatiq_broker, fish_task_manager):
        ...
        print(
            f"{'P99 延迟 (ms)':<25} {fish_metrics.get_percentile(99) * 1000:<20.3f} {dm_metrics.get_percentile(99) * 1000:<20.3f}"
        )
        print(
            f"{'平均延迟 (ms)':<25} {fish_metrics.get_avg_latency() * 1000:<20.3f} {dm_metrics.get_avg_latency() * 1000:<20.3f}"
        )
        print(
            f"{'内存增量 (MB)':<25} {mem_after_fat - mem_before_fat:<20.1f} {mem_after_dm - mem_before_dm:<20.1f}"
        )
        print(
            f"{'成功率 (%)':<25} {fish_metrics.get_success_rate() * 100:<19.1f}% {dm_metrics.get_success_rate() * 100:<19.1f}%"
        )

        # 计算性能差异
        throughput_diff = (
            ((fat_throughput - dm_throughput) / dm_throughput * 100) if dm_throughput > 0 else 0
        )
        print(f"\n性能差异分析:")
    ...
    def test_concurrent_status_query(self, dramatiq_broker, fish_task_manager):
        ...
        print(
            f"{'P50 延迟 (ms)':<25} {fish_metrics.get_percentile(50) * 1000:<20.3f} {dm_metrics.get_percentile(50) * 1000:<20.3f}"
        )
        print(
            f"{'平均延迟 (ms)':<25} {fish_metrics.get_avg_latency() * 1000:<20.3f} {dm_metrics.get_avg_latency() * 1000:<20.3f}"
        )
        ...
        # 计算性能差异
        qps_diff = (
            ((fish_metrics.get_qps() - dm_metrics.get_qps()) / dm_metrics.get_qps() * 100)
            if dm_metrics.get_qps() > 0
            else 0
        )
        print(f"\n性能差异分析:")
    ...
    def test_cleanup_performance(self, dramatiq_broker, fish_task_manager):
        ...
        """
        print_test_header(
            "dramatiq vs FishAsyncTask - 清理操作性能", task_count=TestConfig.TASK_COUNT_LARGE
        )
        ...
        print(
            f"{'清理耗时 (ms)':<25} {fish_cleanup_time * 1000:<20.3f} {dm_cleanup_time * 1000:<20.3f}"
        )
        if dm_throughput > 0:
    ...
    def test_end_to_end_performance(self, dramatiq_broker, fish_task_manager):
        ...
        """
        print_test_header(
            "dramatiq vs FishAsyncTask - 端到端性能", task_count=TestConfig.TASK_COUNT_MEDIUM
        )
        ...
        print(f"{'总耗时 (秒)':<25} {fish_total_time:<20.3f} {dm_total_time:<20.3f}")
        print(
            f"{'任务吞吐量 (tasks/s)':<25} {len(fish_task_ids) / fish_execution_time:<20.0f} {len(dm_messages) / dm_execution_time:<20.0f}"
        )
        print(
            f"{'状态查询 QPS':<25} {fish_query_metrics.get_qps():<20.0f} {dm_query_metrics.get_qps():<20.0f}"
        )
        print(
            f"{'状态查询 P99 (ms)':<25} {fish_query_metrics.get_percentile(99) * 1000:<20.3f} {dm_query_metrics.get_percentile(99) * 1000:<20.3f}"
        )
