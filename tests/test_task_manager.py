...
from fish_async_task.task_manager import TaskQueueFullError
...
def test_cleanup_thread_error_handling():
    ...
    assert (
        task_manager.cleanup_manager.cleanup_thread.is_alive()
    ), "清理线程应该在异常处理后仍在运行"
...
def test_status_update_preserves_fields():
    ...
    task_manager.status_manager.update_task_status(task_id, "running", end_time=time.time())
...
def test_thread_exit_race_condition():
    ...
    if initial_thread_count > task_manager.min_workers:
        assert (
            ...
            or final_thread_count <= task_manager.min_workers + 1
        ), "空闲线程应该已退出"
...
def test_sharded_status_concurrent_updates():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
        futures = [executor.submit(update_task, f"task_{i}", i * 2) for i in range(1000)]
        results = [f.result() for f in futures]
